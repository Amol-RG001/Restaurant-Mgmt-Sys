...
namespace FoodRecipe.xUnitTestProject
{
    ...
    public partial class FoodCategoriesApiTests
    {
        ...
        {
            // ARRANGE
            var dbName = nameof(FoodCategoriesApiTests.UpdateCategory_OkResult01);
            ...
            FoodCategory originalCategory, changedCategory;
            changedCategory = new FoodCategory
            {
                FoodCategoryId = editCategoryId,
                FoodCategoryName = "--Changed Category Name"
            };

            // ACT #1: Get the Record to Edit.

            // (a) Get the Category to edit (to ensure that the row exists before editing it)
            IActionResult actionResultGet = await apiController.GetFoodCategory(editCategoryId);
            ...
            originalCategory = OkResult.Value.Should().BeAssignableTo<FoodCategory>().Subject;
            ...
            try
            {
                ...
                var actionResultPutAttempt1 = await apiController.PutFoodCategory(editCategoryId, changedCategory);
        ...
        {
            // ARRANGE
            var dbName = nameof(FoodCategoriesApiTests.UpdateCategory_OkResult02);
            var logger = Mock.Of<ILogger<FoodCategoriesController>>();
            using var dbContext = DbContextMocker.GetApplicationDbContext(dbName);      // Disposable!
            var apiController = new FoodCategoriesController(dbContext, logger);
            int editCategoryId = 2;
            FoodCategory originalCategory;
            ...
            originalCategory.FoodCategoryName = changedCategoryName;

            // (b) Call the HTTP PUT API to update the changes (with the updated object)
            var actionResultPutAttempt2 = await apiController.PutFoodCategory(editCategoryId, originalCategory);
